...
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, ChatMemberHandler, filters, ContextTypes, JobQueue
...
SAHIP_KANAL_ID = -1004463468496        # 👑 Sahip kontrol kanalı
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ══════════════════════════════════════════════════════
    # SAHIP KANAL — /kanal ve /yaz callback'leri
    # ══════════════════════════════════════════════════════

    elif query.data.startswith('ksec_'):
        # Seçilen kanal için yönetim butonları
        try:
            cid = int(query.data[5:])
        except ValueError:
            await query.answer("❌ Geçersiz kanal ID.", show_alert=True)
            return
        await query.answer()
        data = _kanal_yukle()
        ...
        butonlar = [
            [InlineKeyboardButton("🚪 Bot Kanaldan Çık",        callback_data=f"kyc_{cid}")],
            [InlineKeyboardButton("🔒 Sohbeti Kapat",           callback_data=f"ksk_{cid}"),
             InlineKeyboardButton("🔓 Sohbeti Aç",              callback_data=f"ksa_{cid}")],
            [InlineKeyboardButton("👥 Herkesi At",              callback_data=f"kha_{cid}")],
            [InlineKeyboardButton("🐢 Slow Mode Aç (30s)",      callback_data=f"ksm_{cid}"),
             InlineKeyboardButton("🔔 Slow Mode Kapat",         callback_data=f"ksmk_{cid}")],
            [InlineKeyboardButton("📊 Kanal Bilgisi",           callback_data=f"kbi_{cid}")],
            [InlineKeyboardButton("↩️ Geri",                   callback_data="kgeri")],
        ]
        try:
            await query.edit_message_text(
                f"⚙️ <b>{html.escape(baslik)}</b>\n\nBir işlem seç:",
    ...
    elif query.data.startswith('kyc_'):
        # Bot kanaldan çık
        try:
            cid = int(query.data[4:])
        except ValueError:
        ...
        try:
            ...
            try:
                adminler = await context.bot.get_chat_administrators(cid)
                admin_s = len(adminler)
                bot_s = sum(1 for a in adminler if a.user.is_bot)
            except Exception:
                admin_s = bot_s = "?"
            aciklama = html.escape(chat_b.description or '—')
            tur_emoji = "📢" if chat_b.type == "channel" else "👥"
            msg_text = (
                f"ℹ️ <b>Kanal Bilgisi</b>\n\n"
                f"{tur_emoji} <b>Ad:</b> {html.escape(chat_b.title or '—')}\n"
                f"🆔 <b>ID:</b> <code>{cid}</code>\n"
                f"👥 <b>Üye Sayısı:</b> {uye_s}\n"
                f"👑 <b>Admin Sayısı:</b> {admin_s}\n"
        ...
        if not data:
            try:
                await query.edit_message_text("📭 Bot hiçbir kanalda/grupta değil.")
            except Exception:
                pass
            return
        butonlar = []
    ...
    elif query.data.startswith('ygonder_'):
        # /yaz komutu — seçilen kanala mesaj gönder
        try:
            ...
            return
        await query.answer()
        mesaj = context.bot_data.get('yaz_mesaj_gecici')
        if not mesaj:
            try:
                await query.edit_message_text("❌ Mesaj bulunamadı. Tekrar /yaz <mesaj> kullan.")
            except Exception:
                pass
            return
        try:
            await context.bot.send_message(chat_id=cid, text=mesaj)
            data = _kanal_yukle()
            baslik = data.get(str(cid), {}).get('baslik', str(cid))
            context.bot_data.pop('yaz_mesaj_gecici', None)
            try:
                await query.edit_message_text(
                    f"✅ Mesaj <b>{html.escape(baslik)}</b>'a gönderildi!",
                    parse_mode='HTML'
                )
            except Exception:
                pass
        except Exception as e:
            errmsg = f"❌ Gönderilemedi:\n{html.escape(str(e))}"
            try:
                await query.edit_message_text(errmsg)
            except Exception:
                await context.bot.send_message(chat_id=SAHIP_KANAL_ID, text=errmsg)

    elif query.data == 'yaz_iptal':
        await query.answer()
        context.bot_data.pop('yaz_mesaj_gecici', None)
        try:
...
async def _muzik_ara_ve_goster(metin_mesaj, context, aranan: str):
    """Ortak arama motoru — hem /muzik komutu hem durum tabanlı kullanır."""
    bekle = await metin_mesaj.reply_text("🔍 *Aranıyor...* ⏳", parse_mode="Markdown")
    try:
        ...
        if not sonuclar:
            await bekle.edit_text("❌ Sonuç bulunamadı. Farklı bir şey dene.")
            return

        # Sonuçları user_data'ya kaydet (indirme için)
        user_id = metin_mesaj.from_user.id if metin_mesaj.from_user else 0
        context.user_data[f"muzik_s_{user_id}"] = [
            {"id": e.get("id",""), "title": (e.get("title") or "")[:80]}
            for e in sonuclar[:5]
        ]
        ...
        for i, entry in enumerate(sonuclar[:5], 1):
            ...
            kanal = (entry.get("uploader") or entry.get("channel") or "")[:28]
            metin += f"*{i}.* {baslik}\n"
            if kanal:
        ...
        await bekle.edit_text("❌ *Arama başarısız.* Biraz sonra tekrar dene.", parse_mode="Markdown")


async def muzik_ara_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/muzik [şarkı adı] — YouTube'dan şarkı arar ve MP3 olarak gönderir."""
    aranan = " ".join(context.args).strip() if context.args else ""
    if not aranan:
        await update.effective_message.reply_text(
            "🎵 *MÜZİK ARAMA*\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "Kullanım: `/muzik şarkı adı`\n\n"
            "Örnek: `/muzik Tarkan Kuzu Kuzu`",
            parse_mode="Markdown"
        )
        return
    await _muzik_ara_ve_goster(update.effective_message, context, aranan)


async def muzik_ara_callback(query, context):
    """menu_muzik_ara — kullanıcıdan şarkı adı bekler (durum tabanlı)."""
    await query.answer()
    context.user_data["durum"] = "muzik_arama"
    await query.edit_message_text(
        "🎵 *MÜZİK ARAMA*\n━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "🎤 Hangi şarkıyı arıyorsun?\n\n"
        "_Şarkı adını veya sanatçı adını yaz:_",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([
...
async def muzik_indir_callback(query, context):
    """muzik_indir_0..4 — seçilen şarkıyı indirir ve MP3 gönderir."""
    await query.answer()
    user_id = query.from_user.id
    idx = int(query.data.split("_")[-1])
    sonuclar = context.user_data.get(f"muzik_s_{user_id}", [])
    ...
    try:
        ...
        tmp_dir = tempfile.mkdtemp(prefix="azr_muzik_")
...
async def gelen_mesajlari_yonet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('durum') == 'muzik_arama':
        context.user_data['durum'] = None
        aranan = update.message.text.strip()
        if aranan:
            await _muzik_ara_ve_goster(update.message, context, aranan)
        else:
            await update.message.reply_text("❌ Şarkı adı boş olamaz. Tekrar yaz:")
            context.user_data['durum'] = 'muzik_arama'
        return
...
def _spam_durum_al(chat_id: int, user_id: int) -> dict:
    """{'uyari': 0-5, 'mute': 0-5} döndürür."""
    veri = _spam_mute_yukle()
    kayit = veri.get(str(chat_id), {}).get(str(user_id), {})
    if isinstance(kayit, int):
        # Eski format (sadece sayı) → yeni formata çevir
        kayit = {"uyari": 0, "mute": kayit}
    return {"uyari": kayit.get("uyari", 0), "mute": kayit.get("mute", 0)}

def _spam_durum_kaydet(chat_id: int, user_id: int, uyari: int, mute: int) -> None:
    veri = _spam_mute_yukle()
    ck = str(chat_id)
    uk = str(user_id)
    if ck not in veri:
        veri[ck] = {}
    veri[ck][uk] = {"uyari": uyari, "mute": mute}
    _spam_mute_kaydet(veri)

async def anti_spam_kontrol(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """
    Spam kontrolü.
    Kural: 10 saniye içinde aynı mesajı 3 kez gönder → spam.

    Aşama 1 — Uyarılar (mute YOK):
      1. spam → "Ali spam yapma! 1/5"
      2. spam → "Ali spam yapma! 2/5"
      ...
      5. spam → "Ali spam yapma bu son uyarı!! 5/5"

    Aşama 2 — Muteler (uyarılar doldu):
      6. spam  → 30 dk mute
      7. spam  → 60 dk mute
      8. spam  → 90 dk mute
      9. spam  → 120 dk mute
    ...
    # Hafızayı güncelle
    if chat_id not in _spam_hafiza:
        _spam_hafiza[chat_id] = {}
    if user_id not in _spam_hafiza[chat_id]:
        _spam_hafiza[chat_id][user_id] = []

    _spam_hafiza[chat_id][user_id] = [
        (t, ts) for (t, ts) in _spam_hafiza[chat_id][user_id]
        if simdi - ts < pencere
    ]
    _spam_hafiza[chat_id][user_id].append((metin, simdi))

    ayni_sayisi = sum(1 for (t, _) in _spam_hafiza[chat_id][user_id] if t == metin)
    if ayni_sayisi < esik:
        return False

    # Spam tespit edildi — hafızayı temizle
    _spam_hafiza[chat_id][user_id] = []

    guvenli_isim = html.escape(user.first_name or str(user_id))
    mention = f"<a href='tg://user?id={user_id}'>{guvenli_isim}</a>"
    isim_kisa = html.escape(user.first_name or "Kullanıcı")

    durum = _spam_durum_al(chat_id, user_id)
    uyari = durum["uyari"]
    mute  = durum["mute"]
    ...
    if uyari < 5:
        ...
        if uyari == 5:
            bildirim = (
                f"⚠️ {mention} <b>spam yapma bu son uyarı!! {uyari}/5</b>\n"
                f"Bir daha yaparsan susturulursun! 🔇"
            )
        else:
            bildirim = f"⚠️ {mention} <b>spam yapma! {uyari}/5</b>"

        try:
            await context.bot.send_message(chat_id=chat_id, text=bildirim, parse_mode='HTML')
        except Exception:
            pass
        return True
    ...
    if mute > 5:
        try:
            ...
            ban_metni = (
                f"⛔ {mention} <b>gruptan kalıcı olarak banlandı.</b>\n"
                f"5 kez susturulmasına rağmen spam yapmaya devam etti."
            )
        except Exception as e:
            logger.warning(f"Spam ban hatası ({user_id}@{chat_id}): {e}")
            ban_metni = (
                f"⛔ {mention} ban uygulanamadı (yönetici/kurucu ayrıcalığı).\n"
                f"Ama kayıt tutuldu."
            )
        try:
            await context.bot.send_message(chat_id=chat_id, text=ban_metni, parse_mode='HTML')
        except Exception:
            pass
        return True
    ...
    if sure_dakika >= 60:
        sure_str = (
            f"{sure_dakika // 60} saat {sure_dakika % 60} dakika"
            if sure_dakika % 60 else
            f"{sure_dakika // 60} saat"
        )
    else:
    ...
    if mute_basarili:
        bildirim = (
            f"🔇 {mention} <b>{sure_str} susturuldu.</b>\n"
            f"Sebep: uyarılara rağmen spam yapmaya devam etti.\n"
            f"📊 Susturulma: <b>{mute}/5</b>"
            f"{' — bir sonrakinde ban!' if mute >= 5 else ''}"
        )
    else:
        bildirim = (
            f"⚠️ {mention} susturulamadı (yönetici/kurucu ayrıcalığı).\n"
            f"Ama spam kaydedildi. 📊 Susturulma: <b>{mute}/5</b>"
        )
...
# ══════════════════════════════════════════════════════
# SAHIP KANAL YÖNETİM SİSTEMİ
# ══════════════════════════════════════════════════════

_BOT_KANALLAR_DOSYA = "bot_kanallar.json"


def _kanal_yukle() -> dict:
    try:
        with open(_BOT_KANALLAR_DOSYA, 'r', encoding='utf-8') as f:
            return json.load(f)
    except Exception:
        return {}


def _kanal_kaydet(data: dict):
    try:
        with open(_BOT_KANALLAR_DOSYA, 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)
    except Exception:
        pass


def _kanal_ekle(chat_id: int, baslik: str, tur: str):
    data = _kanal_yukle()
    data[str(chat_id)] = {"id": chat_id, "baslik": baslik or str(chat_id), "tur": tur}
    _kanal_kaydet(data)


def _kanal_sil(chat_id: int):
    data = _kanal_yukle()
...
async def bot_chat_member_degisti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not mcu:
        return
    chat = mcu.chat
    yeni = mcu.new_chat_member.status
    eski = mcu.old_chat_member.status

    # Bot eklendi (member veya administrator oldu)
    if yeni in ('administrator', 'member') and eski in ('left', 'kicked', 'restricted'):
        _kanal_ekle(chat.id, chat.title or str(chat.id), chat.type)

        # Sahibin kontrol kanalı değilse → izinsiz ekleme bildirimi gönder
        if chat.id != SAHIP_KANAL_ID:
            try:
                uye_sayisi = await context.bot.get_chat_member_count(chat.id)
                chat_bilgi = await context.bot.get_chat(chat.id)
                aciklama = html.escape(chat_bilgi.description or '—')
                try:
                    adminler = await context.bot.get_chat_administrators(chat.id)
                    admin_sayisi = len(adminler)
                    bot_sayisi = sum(1 for a in adminler if a.user.is_bot)
                except Exception:
                    admin_sayisi = bot_sayisi = "?"
                tur_emoji = "📢" if chat.type == "channel" else "👥"
                msg = (
                    f"⚠️ <b>!Birisi botu kendi kanalına atmış</b>\n\n"
                    f"{tur_emoji} <b>Kanal Adı:</b> {html.escape(chat.title or '—')}\n"
                    f"🆔 <b>Kanal IDsi:</b> <code>{chat.id}</code>\n"
                    f"📂 <b>Tür:</b> {chat.type}\n\n"
                    f"👥 <b>Kanalda olan kişiler:</b> {uye_sayisi}\n"
                    f"👑 <b>Admin sayısı:</b> {admin_sayisi}\n"
                    f"🤖 <b>Bot sayısı:</b> {bot_sayisi}\n\n"
                    f"📝 <b>Kanal Açıklaması:</b>\n{aciklama}"
                )
                await context.bot.send_message(
                    chat_id=SAHIP_KANAL_ID,
                    text=msg,
                ...
                logger.error(f"İzinsiz kanal bildirimi hatası: {e}")

    # Bot çıkarıldı veya ayrıldı
    elif yeni in ('left', 'kicked'):
        _kanal_sil(chat.id)


async def kanal_listesi_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Botun bulunduğu tüm kanalları/grupları listeler. Sadece SAHIP_KANAL_ID'de çalışır."""
    eff_chat = update.effective_chat
    if not eff_chat or eff_chat.id != SAHIP_KANAL_ID:
        return
    metin, klavye = _kanal_listesi_olustur()
...
async def yaz_komutu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/yaz <mesaj> — mesajı seçilen kanala gönderir. Sadece SAHIP_KANAL_ID'de çalışır."""
    eff_chat = update.effective_chat
    if not eff_chat or eff_chat.id != SAHIP_KANAL_ID:
        return

    # Mesajı doğrudan komuttan al: /yaz Merhaba herkes!
    ...
    if not metin:
        await context.bot.send_message(
            chat_id=SAHIP_KANAL_ID,
            text=(
                "✍️ <b>Kullanım:</b> <code>/yaz Göndermek istediğin mesaj</code>\n\n"
                "<b>Örnek:</b> <code>/yaz Merhaba herkese duyurum var!</code>"
            ),
            parse_mode='HTML'
        )
        return
    ...
    if not data:
        await context.bot.send_message(
            chat_id=SAHIP_KANAL_ID,
            text="📭 Bot şu an hiçbir kanalda/grupta değil."
        )
        return

    # Mesajı bot_data'ya kaydet (chat_data yerine — daha güvenilir)
    context.bot_data['yaz_mesaj_gecici'] = metin
    ...
    for cid_s, info in data.items():
        ...
        tur_emoji = "📢" if info.get('tur') == 'channel' else "👥"
        butonlar.append([
            InlineKeyboardButton(f"{tur_emoji} {baslik}", callback_data=f"ygonder_{cid_s}")
        ])
    butonlar.append([InlineKeyboardButton("❌ İptal", callback_data="yaz_iptal")])

    ozet = metin[:80] + ("…" if len(metin) > 80 else "")
    await context.bot.send_message(
        chat_id=SAHIP_KANAL_ID,
        text=(
            f"📨 <b>Mesajın hazır!</b>\n\n"
            f"📝 <i>{html.escape(ozet)}</i>\n\n"
            f"Hangi kanala/gruba gönderilsin?"
        ),
        reply_markup=InlineKeyboardMarkup(butonlar),
        parse_mode='HTML'
    )
...
async def sahip_kanal_yaz_mesaj_yonet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Artık kullanılmıyor — /yaz komutu inline çalışıyor."""
    pass
...
def main():
    ...
    # ── SAHIP KANAL — /kanal ve /yaz komutları (kanal postları olarak gelir) ──
    application.add_handler(
        MessageHandler(
            filters.UpdateType.CHANNEL_POSTS & filters.Regex(r'^/kanal(@\w+)?(\s|$)'),
            kanal_listesi_komutu
        )
    )
    application.add_handler(
        MessageHandler(
            filters.UpdateType.CHANNEL_POSTS & filters.Regex(r'^/yaz(@\w+)?(\s|$)'),
            yaz_komutu
        )
    )
    # /yaz durumunda gelen serbest metin — diğer handler'lardan önce çalışsın
    application.add_handler(
        MessageHandler(
            filters.UpdateType.CHANNEL_POSTS & filters.TEXT & ~filters.COMMAND,
            sahip_kanal_yaz_mesaj_yonet
        ),
        group=1
    )
    # Bot bir kanala/gruba eklendiğinde veya ayrıldığında tetiklenir
