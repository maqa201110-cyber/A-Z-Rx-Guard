...
def akinator_oyun_baslat(user_id: int, username: str = '', full_name: str = ''):
    """Akinator oyununun başarıyla açıldığını kalıcı olarak kaydeder."""
    with _conn() as c:
...
def akinator_oyun_bitir(
    ...
    """Açık bir Akinator oyununu sonucuyla kapatır.

    sonuc değerleri: ``akinator`` (bot bildi), ``oyuncu`` (bot yanıldı),
    ``abandoned`` (bırakıldı/zaman aşımı/bağlantı hatası).
    """
    if sonuc not in ('akinator', 'oyuncu', 'abandoned'):
        sonuc = 'abandoned'
    soru_sayisi = max(0, int(soru_sayisi or 0))
    sure_saniye = max(0.0, float(sure_saniye or 0))
    with _conn() as c:
...
def akinator_istatistik_getir(user_id: int):
    with _conn() as c:
        return c.execute(
            '''SELECT user_id, username, full_name, games_started,
               ...
               FROM akinator_stats WHERE user_id=?''',
            (int(user_id),)
        ).fetchone()
        ...
        return c.execute(
            '''SELECT user_id, username, full_name, games_started,
               ...
               ORDER BY games_started DESC,
                        (player_wins + akinator_wins) DESC,
